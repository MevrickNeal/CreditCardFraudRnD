/**
 * Friday Fraud - Real-Time Dashboard Controller
 * Updated: 2026-04-15
 */
...
async function loadProfiles() {
    ...
    while (attempts < 10) {
        try {
            ...
            select.innerHTML = '<option value="" data-desc="">-- Choose Card Profile --</option>';
            profiles.forEach(p => {
                select.innerHTML += `<option value="${p.card_number}" data-desc="${p.description}">[${p.type}] Card ending in ${p.card_number.slice(-4)}</option>`;
            });
            select.addEventListener('change', function() {
                const desc = this.options[this.selectedIndex].getAttribute('data-desc');
                document.getElementById('profile-desc-box').innerText = desc || '';
            });
            return;
        } catch (error) {
            attempts++;
...
function showBanner(data) {
    ...
    const iconWarning = `<svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>`;
    const iconCheck = `<svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>`;
    const iconLock = `<svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>`;
    ...
    if (data.status === 'Declined') {
        ...
        document.getElementById('banner-desc').innerText = `Anomaly detected (Score: ${(data.risk_score * 100).toFixed(1)}%). Contextual models blocked this transaction.`;
    ...
    } else if (data.status === 'Verification Required') {
        banner.className = 'glass-panel p-6 border-l-4 border-l-amber-500 rounded-r-2xl mt-6 bg-amber-50/80 fade-in shadow-lg';
        bg.className = 'rounded-2xl p-3 mr-4 bg-amber-500 alert-pulse shadow-md';
        bg.innerHTML = iconLock;
        document.getElementById('banner-title').innerText = 'Bank Verification Required';
        document.getElementById('banner-desc').innerText = `High amount volume detected ($${data.amount}). Held for manual bank review.`;
        ...
        document.getElementById('banner-title').innerText = 'Payment Approved';
...
function prependToFeed(tx) {
    ...
    const iconSuccess = `<svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>`;
    const iconDanger = `<svg class="w-4 h-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>`;
    const iconLock = `<svg class="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>`;

        let icon = iconSuccess;
    let bgClass = 'bg-emerald-50 border-emerald-300 shadow-md ring-2 ring-emerald-400/50';
    let iconWrapperClass = 'bg-emerald-500 border-emerald-600 text-white';
    let textColor = 'text-emerald-700 font-extrabold';
    ...
    } else if (tx.status === 'Verification Required') {
        icon = iconLock;
        bgClass = 'bg-amber-50 border-amber-200';
        iconWrapperClass = 'bg-amber-100 border-amber-200';
    ...
    el.innerHTML = `
        ...
        <div class="text-right">
            ...
            <p class="text-[10px] font-bold uppercase tracking-wide ${textColor}">${tx.status}</p>
        </div>
    `;
